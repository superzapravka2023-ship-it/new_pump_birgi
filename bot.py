"""
Bybit → Bitget  LONG Continuation Signal Bot
============================================
...
MIN_BITGET_GAP_PCT = _f("MIN_BITGET_GAP_PCT", 1.0)    # Bitget должен отставать >= этого, %
GOOD_GAP_PCT       = _f("GOOD_GAP_PCT", 1.5)          # «хороший» запас для +1 балла
...
async def analyze(session, sem, symbol, ticker):
    ...
    # --- Bitget: есть ли монета и насколько отстаёт ---
    async with sem:
        bg_pct = await bitget_move(session, symbol, PUMP_LOOKBACK_MIN)
    if bg_pct is None:
        return None                        # нет на Bitget / нет данных — пропуск
    gap = bybit_pct - bg_pct               # запас хода на Bitget
    if gap < MIN_BITGET_GAP_PCT:
        return None                        # Bitget уже догнал — ловить нечего
    ...
    # 4) комфортный запас на Bitget
    if gap >= GOOD_GAP_PCT:
        score += 1; reasons.append(f"запас {gap:.1f}%")
...
def build_alert(s):
    ...
    return (link,
        ...
        f"🎯 Bitget отстаёт: <b>{s['gap']:.2f}%</b>  (там уже +{s['bg_pct']:.2f}%)\n"
            ...
            for upd in data.get("result", []):
                ...
                if text.startswith("/start"):
                    await tg_send(session,
                        ...
                        f"запас Bitget ≥ <b>{MIN_BITGET_GAP_PCT:.1f}%</b> • "
...
async def main():
    ...
    log.info("Старт LONG-бота. Bybit→Bitget, памп +%.1f%%/%sм, запас≥%.1f%%, балл≥%d",
             PUMP_THRESHOLD_PCT, PUMP_LOOKBACK_MIN, MIN_BITGET_GAP_PCT, MIN_SCORE)
    async with aiohttp.ClientSession() as session:
        await tg_send(session, "🚀 Bybit→Bitget LONG-бот запущен. /status — состояние.")
